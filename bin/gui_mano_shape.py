...
class OnManoChange(OnMano):
    def __init__(self, fig, mbrm):
        ...
        initial_pose = pt.exponential_coordinates_from_transform(
            mbrm.mano2hand_markers_)
        self.pose = initial_pose.copy()
        self.shape = mbrm.hand_state_.betas.copy()
        self.mbrm = mbrm
        self.redraw()
    ...
    def update_mesh(self):
        ...
        mbrm.mano2hand_markers_ = pt.transform_from_exponential_coordinates(self.pose)
        ...
        self.mbrm.hand_state_.vertices[:, :] = pt.transform(
            mbrm.mano2hand_markers_, pt.vectors_to_points(self.mbrm.hand_state_.vertices))[:, :3]
